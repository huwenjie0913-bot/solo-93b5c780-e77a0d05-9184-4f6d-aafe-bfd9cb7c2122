...
function compute(s, rows) {
  ...
  // ---- 逐排 C 值：对所有先前有效排取最小 ----
  // 视线 V(0,vy) 越过第 j 排头顶 H_j(x_j,headY_j)，在 x_i 处的高度
  //   yL = vy + (headY_j-vy)·x_i/x_j
  //   C_ij = eyeY_i - yL
  for (const cur of list) {
    ...
    for (const j of list) {
      ...
      const yL = s.vy + (j.headY - s.vy) * cur.x / j.x;
      const c = cur.eyeY - yL;
    ...
    // 观众眼睛越过 j 排头顶的射线延伸到舞台平面 x=0 处的高度 y0，
    // 舞台面上 0～y0 即为该排被遮挡范围。
    const slope = (j.headY - cur.eyeY) / (j.x - cur.x); // 向舞台方向为负或正
    cur.rayY0 = cur.eyeY + slope * (0 - cur.x);
    cur.blockedH = Math.max(0, cur.rayY0);

    if (best < 0) cur.risk = RISK.BLOCK;
    else if (best < s.cMin) cur.risk = RISK.BAD;
    else if (best < s.cGood) cur.risk = RISK.WARN;
...
function autoGradient() {
  ...
  // 对每个未锁定有效排：对全部先前有效排求满足 C≥目标所需眼位，取最大者。
  // 这样即使中间存在锁定/手改排，也能保证对所有前排在 cGood 上净空充足。
  rows.forEach((r, i) => {
    ...
    for (let j = 0; j < i; j++) {
      ...
      need = Math.max(need, s.vy + (headJ - s.vy) * xs[i] / xs[j] + s.cGood);
    }
    if (need > -Infinity) r.elev = Math.round((need - effEyeOf(s, r.type)) * 1000) / 1000;
  });
...
function drawProfile(c, v, ds, o) {
  ...
  if (o.showLines) {
    L.forEach((row) => {
      ...
      const yL = settings.vy + (j.headY - settings.vy) * row.x / j.x;
      ...
      c.globalAlpha = sel ? 0.95 : 0.28;
      c.setLineDash(sel ? [] : [5, 4]);
      c.beginPath();
      let p = v.P(0, settings.vy); c.moveTo(p[0], p[1]);
      p = v.P(j.x, j.headY); c.lineTo(p[0], p[1]);
      p = v.P(row.x, yL); c.lineTo(p[0], p[1]);
      c.stroke();
      // 眼睛→头顶→舞台面
      ...
      if (sel) {
        // C 值标注：本排眼位与视线之间的竖向小尺
        const [ex, ey] = v.P(row.x, row.eyeY);
        const [, ly] = v.P(row.x, yL);
        c.strokeStyle = "#222"; c.lineWidth = 1.2;
        c.beginPath(); c.moveTo(ex + 6, ey); c.lineTo(ex + 6, ly);
        c.moveTo(ex + 3, ey); c.lineTo(ex + 9, ey);
        c.moveTo(ex + 3, ly); c.lineTo(ex + 9, ly); c.stroke();
        c.fillStyle = "#1d2530"; c.font = "bold 11px sans-serif";
        c.fillText("C=" + r3(row.c), ex + 10, (ey + ly) / 2 + 4);
...
canvas.addEventListener("pointerdown", (e) => {
  ...
  drag = { ...h, moved: false, startX: px, startY: py };
...
canvas.addEventListener("pointermove", (e) => {
  ...
  if (drag && view) {
    ...
    if (drag.kind === "vp") {
      state.settings.vy = clamp(Math.round(w.y * 100) / 100, -1, 3);
      state.settings.firstDistance = clamp(Math.round(w.x * 100) / 100, 0.5, 30);
      syncSettingInputs();
    } else {
      const i = drag.i, row = state.rows[i];
      // 眼位 x_i = 首排距离 + Σ(D_0..D_{i-1})（中点模型中前后半深抵消）
      const prevX = state.settings.firstDistance +
        state.rows.slice(0, i).reduce((a, r) => a + r.depth, 0);
      const newDepth = clamp(Math.round((w.x - prevX) * 100) / 100, 0.45, 3.0);
      row.depth = newDepth;
      // 纵向：仅未锁定的座席/轮椅
      if (!row.locked && row.type !== "aisle") {
        const eye = effEyeOf(state.settings, row.type);
        row.elev = clamp(Math.round((w.y - eye) * 100) / 100, -1, 8);
      }
      syncRowInputs(i);
    }
    persist();
...
function calcBasisLis(s) {
  return `
    ...
    <li><b>C 值校核</b>：对第 n 排，取其先前每一排（通道不计）观众的头顶 H<sub>j</sub>，视线 V→H<sub>j</sub> 在 x<sub>n</sub> 处高度
        y<sub>L</sub> = y<sub>V</sub> + (H<sub>j</sub> − y<sub>V</sub>)·x<sub>n</sub>/x<sub>j</sub>，
        C<sub>n,j</sub> = E<sub>n</sub> − y<sub>L</sub>；取全部 j 中最小值为该排实际 C 值（即最不利遮挡排）。</li>
...
function rowsResultTable(res) {
  const head = `<tr><th>排</th><th>类型</th><th>进深 D(m)</th><th>楼面标高(m)</th>
    <th>水平距 V(m)</th><th>眼位标高(m)</th><th>C 值(m)</th><th>判定</th>
